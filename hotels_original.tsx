...
// Fallback hotel images from Unsplash (used when DB has broken local paths)
...
// Returns a valid image URL — falls back to Unsplash if path is relative/broken
const getHotelImage = (img: string | undefined, index = 0): string => {
  if (!img) return HOTEL_FALLBACKS[index % HOTEL_FALLBACKS.length];
  // If the image is a relative path (starts with /), it's broken on Vercel — use fallback
  if (img.startsWith('/')) return HOTEL_FALLBACKS[index % HOTEL_FALLBACKS.length];
  return img;
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {hotels.slice(0, 4).map((hotel, index) => (
            ...
            >
              <img
                src={getHotelImage(hotel.image || hotel.images?.[0], 0)}
                alt={hotel.title || hotel.name || "Hotel"}
                className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                onError={(e) => { (e.target as HTMLImageElement).src = HOTEL_FALLBACKS[0]; }}
              />
          ...
          <div className="flex-1">
            ...
            >
              ...
              <div className="relative h-64 overflow-hidden">
                <img
                  src={getHotelImage(hotel.image, 0)}
